...
class Site extends IController
{
    ...
    private $redirect='http://www.iwebshop.com/index.php';//回调地址
    private $state='123';//随机数
	...
	function index()
	{
		//获取code,判断地址栏是否有code,有则做登录处理，无说明直接访问首页并未登陆
		
		/*if(isset($_GET['code'])){
			//使用code获取Access token
			$code=$_GET['code']; //code=824C21D4E5ABF1D09A3BDB022E048201
			...
			$left=strpos($str, '=');
			$right=strpos($str, '&');
			$token=substr($str, $left+1,$right-$left-1);//token值：97A45AEA3C2009255FC61A63CB8F9B6F
			//echo $token;
		...
		$ip='47.94.194.205';
		$ak='gUL6kygbfePvc21MZOUGhxoiVoMNrEWA';
	...
	/*function login(){
		...
		header("location:$url");//使用header跳转到$url做表示的地址
	}*/
	//注册
	function reg(){
		$this->redirect('reg');
	...
	function reg_ok(){
		...
		if($id=$user2->add()){
			...
			$title="账号激活";
			$t=time();
			$url="<a href='http://www.iwebshop.com/index.php?controller=site&action=modify&id=$id&t=$t'>http://www.iwebshop.com/index.php?controller=site&action=modify&id=$id&t=$t</a>";
			$content="用户您好这是一封账号激活邮件，请点击网址进行激活".$url."如不是本人操作请忽略";
			if($e->send($email,$title,$content)){
				echo "邮件发送成功";
			}
			else{
				echo "邮件发送失败";
	...
	function modify(){
		...
		$user2=new IModel('user2');
		$status=['status'=>1];
		$user2->setData($status);
		if($user2->update("id=$id")){
			echo "账号激活成功，可以<a href='http://www.iwebshop.com/index.php?controller=site&action=login'>登录</a>";
		}
	}
	function login(){
		$this->redirect("login");
	...
	function login_ok(){
		...
		$pwd=md5(IReq::get('pwd'));
		$user2=new IModel('user2');
		if($user2->getObj("name='$name' and pwd='$pwd' and status=1")){
			echo "登录成功";
		}
		else{
			echo "登录失败";
